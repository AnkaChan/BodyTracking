...
class SMPLSHModel():
  def __init__(self, model_path, useHandPBS=False):
    """
    SMPL model.

    Parameter:
    ---------
    model_path: Path to the SMPL model parameters, npz format
  ...
  def update(self):
    ...
    if self.useHandPBS:
      ...
      lrotmin = (self.R[1:24] - I_cube).ravel()
      v_posed = v_shaped + self.posedirs[:,:,:207].dot(lrotmin)
...
if __name__ == '__main__':
  smpl = SMPLModel('C:/Data/SMPL_python_v.1.0.0/smpl/models/basicModel_m_lbs_10_207_0_v1.0.0.pkl')
  #np.random.seed(10)
  wristJointNum = [21, 20]

  for testJointParam in range(3):
    for i in range(10):
      #pose = (np.random.rand(*smpl.pose_shape) - 0.5) * 0.2
      pose = np.zeros(smpl.pose_shape)
      pose[wristJointNum[0], testJointParam ] = (np.random.rand(1)-0.5)*2
      pose[wristJointNum[1], testJointParam ] = (np.random.rand(1)-0.5)*2
      beta = np.zeros(smpl.beta_shape)
      #beta = (np.random.rand(*smpl.beta_shape) - 0.5) * 3
      trans = np.random.rand(*smpl.trans_shape)
      trans = np.zeros(smpl.trans_shape)
      smpl.set_params(beta=beta, pose=pose, trans=trans)
      smpl.save_to_obj('./TestWristAngle/smpl_np_test' + str(testJointParam) + "_iter" + str(i) + '.obj')
