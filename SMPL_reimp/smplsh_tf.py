...
def smplsh_model(model_path, betas, pose, trans, personalShape=None, simplify=False, unitMM=False, returnDeformedJoints=False):
  ...
  J_regressor = data['JRegressor']
  weights = data['Weights']
  posedirs = data['PoseBlendShapes']
  v_template = data['VTemplate']
  shapedirs = data['ShapeBlendShapes']
  facesArr = data['Faces']
  ...
  results = stacked - \
            pack(
              tf.matmul(
                stacked,
                tf.reshape(
                  tf.concat((J, tf.zeros((numJoints, 1), dtype=tf.float64)), axis=1),
                  (numJoints, 4, 1)
                )
              )
            )
